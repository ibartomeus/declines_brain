...
library(data.table)
namerica <- read_csv("Data/Usa_data/usa_thesis_ma_filtered.csv")
...
ggplot() + geom_map(data = world, map = world,
    ...
    geom_point(data = namerica,aes(long, lat),
...
#Save data
write.csv(all_east_coast, file=gzfile("Data/Usa_data/usa_all_long_lat_thesis.csv.gz"),row.names=FALSE)
